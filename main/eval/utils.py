...
def generate_completions(model, tokenizer, prompts, batch_size=1, stop_id_sequences=None, add_special_tokens=True, disable_tqdm=False, **generation_kwargs):
    ...
    for i in range(0, len(prompts), batch_size):
        ...
        try:
            ...
            # print('input: ', batch_prompts[0])
            print("outputs: ", str(batch_outputs[0][len(batch_prompts[0]):]))
...
def load_hf_lm_and_tokenizer(
    ...
    if gptq_model:
        ...
        if device_map:
            # model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map=device_map, torch_dtype=torch_dtype, output_hidden_states=True)
            model = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map=device_map, output_hidden_states=True)
        else:
            # model = AutoModelForCausalLM.from_pretrained(model_name_or_path, torch_dtype=torch_dtype, output_hidden_states=True)
            model = AutoModelForCausalLM.from_pretrained(model_name_or_path, output_hidden_states=True)
