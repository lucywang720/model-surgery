...
def main(args):
    ...
    examples = []
    prompt_files = glob.glob('data/eval/challenge_prompts.txt')
    for task_file in tqdm(prompt_files, desc="Loading prompts"):
        with open(task_file, "r") as f:
            for d in f.readlines():
                prompt_d = eval(d)
                prompt_d['prompt'] = prompt_d['prompt'].replace('\u201c', '\"').replace('\u201d', '\"').replace('\u2019', '\'')
                examples.append(prompt_d)
    ...
    for example, output, classification in zip(examples, outputs, classifications):
        ...
        all_groups_scores.append(classification)
    with open(os.path.join(args.save_dir, "predictions.jsonl"), "w") as fout:
        for example in examples:
            fout.write(json.dumps(example) + "\n")


    performance = sum(all_groups_scores) / len(all_groups_scores)

    with open(os.path.join(args.save_dir, "metrics.json"), "w") as fout:
        print(f"Average Toxicity: {performance} (by toxigen_roberta)")
