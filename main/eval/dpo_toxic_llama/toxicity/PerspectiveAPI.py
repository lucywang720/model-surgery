    ...
    def request_loop_with_delay(self, queries: Union[List[str], str]):
        ...
        while len(unfulfilled_ids) > 0:
            ...
            for uid, resp in fulfilled.items():
                ...
                # print(resp[0]['attributeScores'])
                toxic += resp[0]['attributeScores']['TOXICITY']['spanScores'][0]['score']['value']
                cnt += resp[0]['attributeScores']['TOXICITY']['spanScores'][0]['score']['value'] > 0.5
                alln += 1
                print(toxic / alln, cnt / alln)
